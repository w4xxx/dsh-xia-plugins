 ...
 */
export function parseKbPath(input) {
    const trimmed = input.trim().replace(/\\/g, '/');
    const firstSlash = trimmed.indexOf('/');
    if (firstSlash <= 0)
        return null;
    const library = trimmed.slice(0, firstSlash);
    if (library === '.' || library === '..' || !/^[\w\u4e00-\u9fff\-\. ]+$/.test(library))
        return null;
 ...
 */
export function resolveLibraryNode(kbRoot, input) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (children.length > 0)
                nodes.push({ kind: 'dir', name, path: `${rel}/`, children });
        }
        else if (entry.isFile() && name.endsWith('.md')) {
            try {
                const st = await stat(abs);
                if (st.size > MAX_NODE_BYTES)
                    continue;
...
export async function scanLibrary(kbRoot, libName) {
...
export async function scanLibraries(kbRoot) {
    let names;
    try {
...
export function renderIndex(libs) {
    if (libs.length === 0)
        return '（还没有知识库：在知识库根目录建一个作品子目录、放入 .md 节点即可）';
    const lines = [];
...
export function renderTree(libs) {
    if (libs.length === 0)
        return '（知识库为空）';
    const lines = [];
    for (const lib of libs) {
    ...
    ctx.effect(() => {
        ...
        const disposeRead = ctx.tools.register({
            ...
            execute: async (args) => {
                ...
                if (resolved === null)
                    return 'path 参数不合法：应形如「库名/目录/节点名」，且不得包含 .. 等越界段。';
                try {
        ...
        const disposeWrite = ctx.tools.register({
            ...
            execute: async (args) => {
                ...
                if (content.length > MAX_NODE_BYTES)
                    return `节点过大（${content.length} 字节），上限 ${MAX_NODE_BYTES} 字节；请拆分成多个节点。`;
                try {

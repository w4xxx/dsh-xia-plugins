...
function usePanelOpen() {
    ...
    React.useEffect(() => {
        const listener = () => { setValue(panelOpen); };
...
function TreeNode(props) {
    ...
    if (node.kind === 'dir') {
        ...
        }, React.createElement('span', { className: styles.caret }, isOpen ? '▾' : '▸'), React.createElement('span', { className: styles.dirName }, `📁 ${node.name}`)), isOpen
            ? (node.children ?? []).map((child) => React.createElement(TreeNode, {
                key: child.path,
                library,
                node: child,
                depth: depth + 1,
    ...
    }, React.createElement('span', { className: styles.fileName }, `📄 ${node.title ?? node.name}`), React.createElement('span', { className: styles.actions }, React.createElement('button', {
        type: 'button',
...
function KnowledgePanel(props) {
    ...
    const [libraries, setLibraries] = React.useState(() => treeCache ?? []);
    ...
    if (!open)
        return null;
    if (props.session?.blank === true)
        return null;
    const library = libraries.find(item => item.name === active) ?? libraries[0];
    return React.createElement('div', { className: styles.panel }, React.createElement('div', { className: styles.head }, React.createElement('span', { className: styles.title }, '📚 知识库'), libraries.length > 1
        ? React.createElement('div', { className: styles.tabs }, libraries.map(item => React.createElement('button', {
            key: item.name,
            type: 'button',
        ...
        title: '刷新树',
        onClick: () => {
            void fetchTree().then(next => { if (next !== null)
                setLibraries(next); });
        },
    }, '⟳')), library === undefined
        ? React.createElement('p', { className: styles.hint }, '（还没有知识库节点。在知识库根目录建一个作品子目录、放入 .md 文件即可）')
        : React.createElement('div', { className: styles.tree }, library.nodes.map((node) => React.createElement(TreeNode, {
            key: node.path,
            library: library.name,
            node,
            depth: 0,
            expanded,
            onToggle: (path) => { setExpanded(prev => ({ ...prev, [path]: !(prev[path] ?? false) })); },
            onCitePath: citePath,
            onCiteFull: citeFull,
            onDragStart: (payload) => { setDragging(payload); },
        }))), notice !== null ? React.createElement('div', { className: styles.notice }, notice) : null, dragging !== null
...
export function apply(ctx) {
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({ name: 'conversation.input.left', id: 'gameassist-knowledge', order: 20 }, KnowledgeButton));
    ctx.slots.inject('conversation.input.dock', () => ctx.slots.register({ name: 'conversation.input.dock', id: 'gameassist-knowledge-panel', order: 10 }, KnowledgePanel));
}

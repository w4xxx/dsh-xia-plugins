...
export function pickFor(ids, key) {
    let hash = 0;
...
export function renderCard(card) {
    const lines = [`【${card.name}】${card.source === undefined ? '' : ` —— 出自 ${card.source}`}`];
    if (card.cv !== undefined)
        lines.push(`声优：${card.cv}`);
    if (card.role !== undefined)
        lines.push(`游戏开发职责：${card.role}`);
    if (card.appearance !== undefined)
        lines.push(`外貌：${card.appearance}`);
    if (card.personality !== undefined && card.personality.length > 0)
        lines.push(`性格：${card.personality.join('；')}`);
    if (card.speech !== undefined) {
        if (card.speech.callsUser !== undefined)
            lines.push(`称呼主人：${card.speech.callsUser}`);
        if (card.speech.style !== undefined)
            lines.push(`说话风格：${card.speech.style}`);
        if (card.speech.catchphrases !== undefined && card.speech.catchphrases.length > 0)
            lines.push(`口头禅：${card.speech.catchphrases.join('、')}`);
    }
    if (card.devSkill !== undefined)
        lines.push(`开发专长：${card.devSkill}`);
    if (card.playbook !== undefined && card.playbook.length > 0)
        lines.push(`扮演要点：\n- ${card.playbook.join('\n- ')}`);
    if (card.taboo !== undefined && card.taboo.length > 0)
        lines.push(`禁止事项：\n- ${card.taboo.join('\n- ')}`);
    return lines.join('\n');
}
/**
        ...
        const disposeRoute = ctx.webServer.register({
            ...
            handler: (_req, res) => {
                ...
                for (const card of cards) {
                    if (card.voice !== undefined)
                        voices[card.id] = card.voice;
                }
                const today = current();
        ...
        const disposePick = ctx.tools.register({
            ...
            execute: async (args) => {
                ...
                if (args.id !== undefined) {
                    ...
                    if (found === undefined)
                        return `没有找到角色 "${args.id}"。可用：${cards.map(card => card.id).join('、')}`;
                    overrideId = found.id;
                }
                else {

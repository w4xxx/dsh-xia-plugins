...
const SAKURA_TOKENS: Record<string, { light: string; dark: string }> = {
...
const GREETINGS: ReadonlyArray<readonly [number, number, string]> = [
  [5, 11, '早上好~今天也要元气满满'],
  [11, 17, '下午好~小夏陪你敲代码'],
...
function saveVoicePref(pref: VoicePref): void {
  try { window.localStorage.setItem(VOICE_STORAGE_KEY, JSON.stringify(pref)) } catch { /* non-persistent */ }
}
...
/** Per-day voice tweak for today's companion role. */
...
function loadRoleOverride(): RoleOverride | null {
  try {
    const raw = window.localStorage.getItem(ROLE_OVERRIDE_KEY)
    if (raw === null) return null
    const parsed = JSON.parse(raw) as Partial<RoleOverride>
...
function saveRoleOverride(value: RoleOverride): void {
  try { window.localStorage.setItem(ROLE_OVERRIDE_KEY, JSON.stringify(value)) } catch { /* non-persistent */ }
}

/** Drop the role-voice override (back to the card defaults). */
function clearRoleOverride(): void {
  try { window.localStorage.removeItem(ROLE_OVERRIDE_KEY) } catch { /* non-persistent */ }
}

/** Voice map served by the host roster plugin: today's character plus per-character voice hints. */
interface CardVoice { name?: string; voiceURI?: string; lang?: string; rate?: number; pitch?: number }
interface VoiceMap { today: string | null; cardName?: string | null; voices: Record<string, CardVoice> }
let voiceMap: VoiceMap | null = null
...
async function speakText(text: string, options?: { onend?: () => void }): Promise<{ ok: boolean; voiceName: string }> {
  ...
  try {
    ...
    const pickUri = (uri: string): SpeechSynthesisVoice | undefined =>
      voices.find((item) => item.voiceURI === uri)
    const pickName = (name: string): SpeechSynthesisVoice | undefined => {
      const needle = name.toLowerCase()
      return voices.find((item) => item.name.toLowerCase().includes(needle))
    }
    const pickLang = (lang: string): SpeechSynthesisVoice | undefined => {
    ...
    if (targetURI !== null && targetURI !== undefined) chosen = pickUri(targetURI)
    if (chosen === undefined && pref.voiceURI !== null && pref.voiceURI !== '') chosen = pickUri(pref.voiceURI)
    if (chosen === undefined && pref.voiceName !== '') chosen = pickName(pref.voiceName)
    if (chosen === undefined && targetName !== undefined && targetName !== '') chosen = pickName(targetName)
    if (chosen === undefined) chosen = pickLang(char?.lang ?? 'zh-CN')
    if (chosen === undefined) chosen = pickLang('zh')
    if (chosen !== undefined) {
    ...
    if (options?.onend !== undefined) utter.onend = options.onend
    synth.speak(utter)
...
async function speakDefaultText(text: string): Promise<void> {
  ...
  if (pref.endpoint !== '') {
    try {
      ...
      const url = URL.createObjectURL(blob)
      const audio = new Audio(url)
      const finish = (): void => { URL.revokeObjectURL(url) }
      audio.onended = finish
      audio.onerror = finish
      await audio.play()
      return
  ...
  try {
    ...
    if (chosen === undefined && pref.voiceName !== '') chosen = pickName(pref.voiceName)
    if (chosen === undefined) chosen = pickLang('zh-CN')
    if (chosen === undefined) chosen = pickLang('zh')
    if (chosen !== undefined) {
      if (chosen.lang !== '') utter.lang = chosen.lang
      utter.voice = chosen
...
function VoiceSettings(): any {
  ...
  const [pref, setPref] = React.useState<VoicePref>(loadVoicePref)
  const [map, setMap] = React.useState<VoiceMap | null>(voiceMap)
  const [override, setOverride] = React.useState<RoleOverride | null>(() => {
    const stored = loadRoleOverride()
    const today = voiceMap?.today ?? null
    if (stored === null || today === null || stored.date !== localDateKey(new Date()) || stored.cardId !== today) return null
    return stored
  })
  ...
  const update = (next: Partial<VoicePref>): void => {
  ...
  const updateRole = (next: Partial<RoleOverride>): void => {
  ...
  return React.createElement('div', { className: styles.voicePage },
    ...
    React.createElement('label', { className: styles.voiceField }, '音色',
      React.createElement('select', {
        ...
        React.createElement('option', { value: '' }, '角色卡预设（自动）'),
        roleURI !== '' && !listed.some((item) => item.voiceURI === roleURI)
          ? React.createElement('option', { value: roleURI }, '已保存的音色（当前浏览器不可用）')
          : null,
        listed.map((item) => React.createElement('option', { key: item.voiceURI, value: item.voiceURI }, `${item.name}（${item.lang}）`)),
      ),
    ),
    React.createElement('label', { className: styles.voiceField }, `语调 ${rolePitch.toFixed(1)}`,
    ...
    React.createElement('label', { className: styles.voiceField }, `语速 ${roleRate.toFixed(1)}`,
      React.createElement('input', {
        className: styles.voiceRange,
        type: 'range',
        min: 0.5,
    ...
    React.createElement('h4', { className: styles.voiceSection }, '默认语音（其他模式）'),
    React.createElement('p', { className: styles.voiceHint },
      '非轮值场景（如 standard 模式）的消息朗读与提醒使用这里的音色；小夏模式下角色没有预设时也回退到这里。'),
    React.createElement('label', { className: styles.voiceField }, '音色',
      React.createElement('select', {
        className: styles.voiceSelect,
        value: pref.voiceURI ?? '',
        ...
        React.createElement('option', { value: '' }, '自动选择（首选中文语音）'),
        pref.voiceURI !== null && !listed.some((item) => item.voiceURI === pref.voiceURI)
          ? React.createElement('option', { value: pref.voiceURI }, '已保存的音色（当前浏览器不可用）')
          : null,
        listed.map((item) => React.createElement('option', { key: item.voiceURI, value: item.voiceURI }, `${item.name}（${item.lang}）`)),
      ),
      pref.voiceURI !== null && voices.length > 0 && !voices.some((item) => item.voiceURI === pref.voiceURI)
        ? React.createElement('p', { className: styles.voiceHint }, '注意：已保存的音色在当前浏览器不可用（可能换了浏览器或未安装对应语音包），朗读将回退到自动选择。')
        : null,
    ),
    React.createElement('label', { className: styles.voiceField }, `语速 ${pref.rate.toFixed(1)}`,
      React.createElement('input', {
        className: styles.voiceRange,
        type: 'range',
        min: 0.5,
    ...
    React.createElement('label', { className: styles.voiceField }, `音调 ${pref.pitch.toFixed(1)}`,
      React.createElement('input', {
        className: styles.voiceRange,
        type: 'range',
        min: 0.5,
    ...
    React.createElement('label', { className: styles.voiceField }, '自定义 TTS 端点（可选：开源引擎自托管，如 Kokoro / Piper / ChatTTS）',
      React.createElement('input', {
        className: styles.voiceSelect,
        type: 'text',
    ...
    React.createElement('p', { className: styles.voiceHint }, '设置保存在本浏览器；消息朗读按钮与审批提醒语音都会使用这里的音色。'),
  )
}
 ...
 * assistant rows.
 */
function UserReadAloudAction(props: any): any {
...
export function apply(ctx: any): void {
  ...
  ctx.slots.inject('conversation.composer.dock', () => {
    const disposeCharm = ctx.slots.register(
      { name: 'conversation.composer.dock', id: 'assistant-charm', order: 50 },
      () => React.createElement('div', { className: styles.charm },
        React.createElement('span', { className: styles.dot }),
        React.createElement('span', null, '小夏在线 · ' + greeting()),
      ),
    )
    ...
    function QuestionNotifier(props: any): any {
      ...
      React.useEffect(() => () => {
        if (timers.current.remind !== undefined) window.clearInterval(timers.current.remind)
      }, [])
      React.useEffect(() => {
        if (!pending) {
          timers.current.reminded.clear()
          if (timers.current.remind !== undefined) { window.clearInterval(timers.current.remind); timers.current.remind = undefined }
          return
        }
      ...
      return React.createElement('div', { className: styles.notifyCharm },
        React.createElement('span', null, '❓ 小夏在等主人做决定'),
      )
    }
     ...
     * interactions).
     */
    function JobNotifier(props: any): any {
    ...
    function ApprovalNotifier(props: any): any {
      ...
      React.useEffect(() => () => {
        if (timers.current.remind !== undefined) window.clearInterval(timers.current.remind)
        if (timers.current.stop !== undefined) window.clearTimeout(timers.current.stop)
      }, [])
      React.useEffect(() => {
        if (!pending) {
          timers.current.reminded.clear()
          if (timers.current.remind !== undefined) { window.clearInterval(timers.current.remind); timers.current.remind = undefined }
          if (timers.current.stop !== undefined) { window.clearTimeout(timers.current.stop); timers.current.stop = undefined }
          return
        }
        const fresh = String(pendingKey).split(',').filter((key) => !timers.current.reminded.has(key))
        if (fresh.length > 0) {
      ...
      if (!pending) return null
      return React.createElement('div', { className: styles.notifyCharm },
        React.createElement('span', null, '🔔 有授权请求等主人确认 · 超时自动停止'),
      )
    }

    /** Stop the in-flight turn (the composer stop-button path); reject leftovers on failure. */
    async function stopTurn(sessionId: any): Promise<void> {
      try {
        const binding = ctx.sessions.binding(sessionId)
        if (binding === undefined) return
        const result = await binding.session.cancel()
        if (result !== undefined && result.ok !== true) throw new Error('cancel returned not-ok')
      } catch {
        try {
          const binding = ctx.sessions.binding(sessionId)
    ...
    const disposeNotify = ctx.slots.register(
      { name: 'conversation.composer.dock', id: 'assistant-approval-notify', order: 60 },
      ApprovalNotifier,
    )
    const disposeQuestion = ctx.slots.register(
      { name: 'conversation.composer.dock', id: 'assistant-question-notify', order: 61 },
      QuestionNotifier,
    )
    const disposeJob = ctx.slots.register(
      { name: 'conversation.composer.dock', id: 'assistant-job-notify', order: 62 },
      JobNotifier,
    )
    const disposeAnswer = ctx.slots.register(
      { name: 'conversation.composer.dock', id: 'assistant-answer-done-notify', order: 63 },
      AnswerDoneNotifier,
    )
    return () => {
      disposeCharm()
  ...
  ctx.slots.inject('conversation.chat.assistant-actions', () => ctx.slots.register(
    { name: 'conversation.chat.assistant-actions', id: 'read-aloud', order: 5 },
    ReadAloudAction,
  ))

  ctx.slots.inject('conversation.chat.user-actions', () => ctx.slots.register(
    { name: 'conversation.chat.user-actions', id: 'read-aloud', order: 5 },
    UserReadAloudAction,
  ))

  ctx.slots.inject('settings.section', () => ctx.slots.register(
    { name: 'settings.section', id: 'voice', order: 20, label: '语音朗读' },
    VoiceSettings,
  ))

  ctx.slots.inject('shell.overlay', () => ctx.slots.register(
    { name: 'shell.overlay', id: 'sakura-petals', order: -100 },
    () => React.createElement('div', { className: styles.petalLayer, 'aria-hidden': true },
      Array.from({ length: 8 }, (_, index) => React.createElement('span', {
        key: index,
